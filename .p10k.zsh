# Generated by Powerlevel10k configuration wizard on 2025-06-02 at 23:13 EAT.
# Based on romkatv/powerlevel10k/config/p10k-rainbow.zsh, checksum 49619.
# Wizard options: awesome-fontconfig + powerline + python, large icons, rainbow, ascii,
...
# Set the shorten strategy to none to avoid shortening intermediate segments.
typeset -g POWERLEVEL9K_SHORTEN_STRATEGY=none
# Remove the delimiter for shortened segments (not needed since we're not shortening).
typeset -g POWERLEVEL9K_SHORTEN_DELIMITER=
# Color of the shortened directory segments (not applicable here, but kept for consistency).
typeset -g POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=250
# Color of the anchor directory segments (last folder will be an anchor).
...
# Set to "last" to show only the last subdirectory.
typeset -g POWERLEVEL9K_DIR_TRUNCATE_BEFORE_MARKER=last
# Keep only the last directory segment as an anchor.
typeset -g POWERLEVEL9K_SHORTEN_DIR_LENGTH=1
# Set max length to a reasonable value (optional, can be adjusted).
typeset -g POWERLEVEL9K_DIR_MAX_LENGTH=80
# Ensure enough space for typing commands.
typeset -g POWERLEVEL9K_DIR_MIN_COMMAND_COLUMNS=40
# Percentage of terminal width for command columns.
typeset -g POWERLEVEL9K_DIR_MIN_COMMAND_COLUMNS_PCT=50
# Disable hyperlink for simplicity (optional).
